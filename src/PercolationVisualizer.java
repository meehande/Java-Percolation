...
public class PercolationVisualizer {
    ...
    public static void main(String[] args) {
        ...
        In in = new In("D:\\AlgsCourse\\Assignment1-Percolation-Submission\\percolation-test-data\\input10.txt");      // input file
        ...
        StdDraw.pause(DELAY);
        int counter = 0;
        while (!in.isEmpty() && !percolated) {
        	counter++;
            int i = in.readInt();
            int j = in.readInt();
            perc.open(i, j);
            draw(perc, n);
            percolated = perc.percolates();
            isFull = perc.isFull(i,  j);
            StdDraw.show();
            StdDraw.pause(DELAY);
        }
    }
